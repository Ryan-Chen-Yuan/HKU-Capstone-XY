...
请确保分析客观、专业，提供具体可行的建议。"""
    ...
    def generate_user_report(
        self, user_id: str, session_ids: List[str], time_period: int = 30
    ) -> Dict[str, Any]:
        """生成用户综合分析报告
        ...
        try:
            ...
            report = {
                ...
                "analysis_period": {
                    "days": time_period,
                    "start_date": (
                        datetime.now() - timedelta(days=time_period)
                    ).isoformat(),
    ...
    def _collect_user_data(
        self, user_id: str, session_ids: List[str], time_period: int
    ) -> Dict[str, Any]:
        """收集用户相关数据
        ...
        for i, session_id in enumerate(session_ids, 1):
            ...
            if filtered_events:
                ...
                for j, event in enumerate(filtered_events[:3], 1):  # 只显示前3个事件
                    print(
                        f"      事件{j}: {event.get('primaryType', '未知类型')} - {event.get('subType', '未知子类型')}"
                    )
                    print(f"        时间: {event.get('time', '未知时间')}")
                    print(
                        f"        描述: {event.get('description', '无描述')[:100]}..."
                    )
    ...
    def _analyze_events(self, events: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            "event_status": Counter(
                [event.get("status", "unknown") for event in events]
            ),
        }
    ...
    def _analyze_emotions(self, emotions: List[Dict]) -> Dict[str, Any]:
        ...
        # 情绪类别统计
        emotion_categories = [
            emotion.get("emotion_category", "unknown") for emotion in emotions
        ]
    ...
    def _compare_patterns(self, prev_pattern: Dict, curr_pattern: Dict) -> List[str]:
        ...
        # 比较行为模式的主要字段
        if (
            "behavioral_patterns" in prev_pattern
            and "behavioral_patterns" in curr_pattern
        ):
    ...
    def _calculate_engagement_score(self, user_data: Dict[str, Any]) -> float:
        ...
        scores = {
            "events": min(
                len(user_data.get("events", [])) / 10, 1.0
            ),  # 最多10个事件得满分
            "patterns": min(
                len(user_data.get("patterns", [])) / 5, 1.0
            ),  # 最多5个模式得满分
            "emotions": min(
                len(user_data.get("emotions", [])) / 20, 1.0
            ),  # 最多20个情绪记录得满分
            "memory": min(
                len(user_data.get("long_term_memory", [])) / 10, 1.0
            ),  # 最多10个记忆得满分
            "profile": (
                1.0 if user_data.get("user_profile") else 0.0
            ),  # 有用户画像得满分
        }
    ...
    def _generate_ai_analysis(
        self, user_data: Dict[str, Any], statistics_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """使用AI生成深度分析
        ...
        try:
            ...
            analysis_input = {
                "user_data_summary": {
                    ...
                    "total_patterns": len(user_data.get("patterns", [])),
                    "total_emotions": len(user_data.get("emotions", [])),
                    "engagement_score": statistics_data.get(
    ...
    def _format_report_as_text(self, report: Dict[str, Any]) -> str:
        ...
        text_report.append(
            f"分析周期: {report.get('analysis_period', {}).get('days', 'Unknown')}天"
        )
        text_report.append("")
