...
请保持JSON格式的完整性。"""
    ...
    def batch_get_user_data(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            futures = {
                ...
                'memory': self.executor.submit(self.db.get_long_term_memory, user_id, 5),
                'emotion_history': self.executor.submit(self.db.get_emotion_history, user_id)
            }
    ...
    def format_memory_context(self, memories: List[Dict[str, Any]]) -> str:
        ...
        if not memories:
            return ""
        return "\n".join(f"{m.get('time', '')}: {m.get('content', '')}" for m in memories)
...
def detect_crisis(state: OptimizedSessionState) -> OptimizedSessionState:
    ...
    if crisis:
        ...
        chat_service.db.save_long_term_memory(
            state.user_id, 
            f"[CRISIS-{severity.upper()}] {state.user_input} – {reason}"
        )
...
def build_context(state: OptimizedSessionState) -> OptimizedSessionState:
    ...
    state.memory_context = chat_service.format_memory_context(user_data.get('memory', []))
    ...
    try:
        ...
        save_futures.append(
            chat_service.executor.submit(
                ...
                state.user_id, state.session_id, emotion_score, state.emotion
            )
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "test":
        ...
        while True:
            ...
            result = optimized_chat(user_input, history=history, enable_performance_monitoring=enable_perf)
