...
class RAGCoreService:
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if new_files:
                ...
                logger.warning("尝试重新扫描所有文件...")
                
                # 清理已有的processed标记，重新处理所有文件
                ...
                if new_files:
                    processed_count = self._process_new_files(new_files)
                    logger.info(f"重新处理 {processed_count} 个文件")
                    self.vector_store.save_index()
                else:
    ...
    def _process_new_files(self, new_files: List[DocumentInfo]) -> int:
        ...
        print(f"📁 开始处理 {len(new_files)} 个新文件...")
        
        # 添加进度条用于文档内容提取
        for doc_info in tqdm(new_files, desc="提取文档内容", unit="个"):
            try:
                # 提取文档内容
        ...
        if documents:
            try:
                print(f"📊 开始向量化 {len(documents)} 个有效文档...")
                chunks_count = self.vector_store.add_documents(documents)
                
                # 标记文件为已处理
                print("📝 标记文件为已处理...")
                for doc_info in tqdm(new_files, desc="更新状态", unit="个"):
                    # 从原始文档中查找对应的内容来计算chunk数量
                    content = self.scanner.extract_document_content(doc_info)
                    if content.strip():
                        # 估算这个文件的chunk数量
                ...
                print(f"✅ 文件处理完成: {processed_count} 个文件，{chunks_count} 个文档块")
    ...
    def force_rebuild_index(self):
        ...
        if all_files:
            ...
            print(f"✅ 索引重建完成: 处理了 {processed_count} 个文件")
